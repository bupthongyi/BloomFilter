...
namespace detail {
	...
	struct BloomOp 
	{
		...
		void update(typename CBF::bucket_type& slots,const size_t num_bins,const size_t limit)
		{
			static Op op;
			...
			for (size_t i = 0; i < N; ++i) 
			{
				...
				const size_t offset_bits = (hash % CBF::bins_per_slot()) * CBF::bits_per_bin();
				const size_t target_bits =(slots[pos] >> offset_bits) & CBF::mask();
	...
	struct twohash_counting_apply_hash
	{
						   ...
						   const size_t num_bins)
		{
			...
			inserter.update(slots, num_bins, (static_cast<size_t>(1) << CBF::bits_per_bin()) - 1);
		}
