...
namespace bloom_filters {
	...
	public:
      //* constructors
		twohash_dynamic_basic_bloom_filter() : bits(default_size)
		{
		}

		explicit twohash_dynamic_basic_bloom_filter(const size_t size): bits(size)
		{
		}

		template <typename InputIterator>
		twohash_dynamic_basic_bloom_filter(const InputIterator start, const InputIterator end)
		...
		template<class _T, 
				...
				typename _Block, 
				class _Allocator>
		friend bool operator==(const twohash_dynamic_basic_bloom_filter<_T, 
																		_HashValues, 
    ...
    template<class T, 
			...
			typename Block, 
			class Allocator>
	bool operator==(const twohash_dynamic_basic_bloom_filter<T, 
															HashValues,
															ExpectedInsertionCount,
															HashFunction1,
															HashFunction2,
